...
The config file must be valid JSON and must have a `.json` extension. By default the generator looks for `openapi-zod-ts.config.json` in the current working directory.
...
<Aside type="note">
  The `--config` value must end in `.json`. Any other extension is rejected with an error. When
  `--config` is used, all relative paths inside the config file resolve from the config file's
...
The generator looks for `openapi-zod-ts.config.json` in the current working directory. If you run the CLI from a subdirectory or use a different file name, pass `--config <path>` explicitly. The path argument must end in `.json`.
