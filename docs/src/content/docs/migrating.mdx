...
2. **Install `openapi-zod-ts`.**
   ...
   If your old setup relied on MSW or Faker mocks (orval and kubb both ship mock
...
**OpenAPI 3.0.x is supported in practice.** OpenAPI 3.1 (including 3.1.1) is the primary target, and 3.0.x works too: 8 of the 13 showcase specs are 3.0.x and all compile under `tsc --strict`. For 3.0.x specs, `nullable: true` is normalized to the 3.1 null-union form (`type: [..., "null"]`) before generation runs, so nullable fields produce the correct `T | null` types. A few 3.0-only constructs are still being normalized to 3.1 semantics.
...
<CardGrid>
  ...
  <Card title="Mock data with MSW" icon="random">
    Generate MSW v2 handlers with seeded Faker data from the same spec. The
    replacement for orval or kubb mock generation.
