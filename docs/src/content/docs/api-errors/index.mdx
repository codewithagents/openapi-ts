...
<Aside type="note">
  `extractErrors` is the recommended entry point for new code. `extractFieldErrors` remains available
  for backwards compatibility. The only behavioural difference is that `extractErrors` returns
  form-level errors in a dedicated `formErrors` channel, while `extractFieldErrors` folds them into
  `fieldErrors` under `fallbackField`.
</Aside>
