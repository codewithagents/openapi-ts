...
The generated `client.ts` also calls `CreatePetRequestSchema.parse(body)` before sending `POST /pets`, and `PetSchema.parse(await res.json())` after every response, so type drift between client and server is caught at runtime.
...
The validation error test is the full round-trip in action: the React form submits empty strings, the Hono router's `CreatePetRequestSchema.safeParse` rejects them and returns `422` with the Zod issue list, the generated client throws an `ApiError` (status 422, body `{ issues: [...] }`), the `onError` callback in `useCreatePet` runs `parseValidationErrors`, and the component renders the field-level error messages.
