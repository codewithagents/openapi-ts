...
- Runs on every PR that touches `packages/openapi-zod-ts/**`, `packages/openapi-react-query/**`, `packages/openapi-server/**`, or `examples/**`, plus a weekly run on Monday at 6am UTC to catch upstream spec drift.
