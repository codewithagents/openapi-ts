...
| [`@codewithagents/openapi-server`](/openapi-server)           | Generates a typed service interface and optional Hono, Express, or Fastify router  |
...
- Node.js 18 or later
- An OpenAPI 3.1 spec (JSON or YAML). OpenAPI 3.0.x is well-supported in practice: 8 of the 13 showcase specs are 3.0.x and all compile. A few 3.0-only constructs like `nullable` are not yet normalized to 3.1 semantics.
- TypeScript with `strict: true` recommended (all generated output passes it out of the box)

<Aside type="tip">
  Start with only `openapi-zod-ts`. Get a typed client working, then layer in hooks or
  a server interface when you need them.
</Aside>
