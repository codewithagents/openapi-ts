...
<Aside type="note">
  ...
  pagination param such as `page`, `cursor`, or `offset`) also get a generated `useXxxInfinite`
  hook powered by React Query's `useInfiniteQuery`. Provide `getNextPageParam` and
  `initialPageParam` via `options` to activate real pagination. Shipped in 3.11.0 (#189).
</Aside>
...
export function useDeleteTask(
  options?: Omit<
    UseMutationOptions<Awaited<ReturnType<typeof deleteTask>>, ApiError, Parameters<typeof deleteTask>[0]>,
    'mutationFn'
  >
) {
  return useMutation<Awaited<ReturnType<typeof deleteTask>>, ApiError, Parameters<typeof deleteTask>[0]>({
