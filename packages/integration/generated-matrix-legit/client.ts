...
import type { LegitEmptyObject, LegitOpenMap } from './models.js'
...
export async function postOpenMap(
  body: LegitOpenMap,
  config?: Partial<ClientConfig>
): Promise<LegitOpenMap> {
  const res = await _request('POST', '/legit/open-map', { body }, config)
  return res.json()
}

export async function postEmptyObject(
  body: LegitEmptyObject,
  config?: Partial<ClientConfig>
): Promise<LegitEmptyObject> {
  const res = await _request('POST', '/legit/empty-object', { body }, config)
  return res.json()
}
